module Comments
  def self.require_user_can_comment_on_content(user, object)
    ...
    if user.antiflood_level > -1 then
      ...
      # esto debería estar fuera pero como el usuario está baneado de la facción que se joda, no hacemos 2 queries por culpa de un 1% de los usuarios
  ...
  def self.user_can_moderate_comments_of_content(user, content)
    ...
    #  true
    sif real.class.name == 'Event' && (cm = CompetitionsMatch.find_by_event_id(real.id)) && cm.competition.user_is_admin(user.id)
      true
