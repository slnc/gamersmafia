...
module ActsAsContent
  ...
  module AddActsAsContent
    def acts_as_content # necesario para registrar los distintos callbacks
      ...
      serialize :log
      observe_attr :log, :state, :user_id
      attr_accessor :cur_editor
      
      before_save do |m| 
        if !m.slnc_changed?(:log) 
          if m.cur_editor
  ...
  module InstanceMethods
    ...
    def do_before_save      
      ...
      # if (self.respond_to?(:title) && slnc_changed?(:title)) 
      # TODO más inteligencia?
      # creamos versión si se ha cambiado title, description, main o el campo de categoría
