...
class PublishingDecisionTest < ActiveSupport::TestCase
  ...
  def test_users_exp_doesnt_change_when_he_accepts_a_content
  ...
  def test_users_exp_doesnt_change_when_he_denies_a_content
    exp = @panzer_personality.experience
  ...
  def test_users_exp_increases_when_a_content_he_accepted_is_published
    exp = @panzer_personality.experience
  ...
  def test_users_exp_increases_when_a_content_he_denied_and_became_denied_changes_and_now_is_published
    exp = @panzer_personality.experience
    assert_equal 0.0, exp
  ...
  def test_users_exp_decreases_when_a_content_he_accepted_is_denied
    exp = @panzer_personality.experience
    assert_equal 0.0, exp
    make_a_decision :publish_content, @panzer
  ...
  def test_users_exp_decreases_when_a_content_he_rejected_is_published
    exp = @panzer_personality.experience
    assert_equal 0.0, exp
    make_a_decision :deny_content, @panzer, 'feo'
    make_a_decision :publish_content, @superadmin
  ...
  def test_power_user_can_publish_directly
    n = News.find(:first, :conditions => 'clan_id is null and state = 1', :order => 'id DESC')
    make_a_decision :publish_content, @superadmin
    n.reload
    assert_equal Cms::PUBLISHED, n.state
  ...
  def test_power_user_can_deny_directly
    n = News.find(:first, :conditions => 'clan_id is null and state = 1', :order => 'id DESC')
    make_a_decision :deny_content, @superadmin, 'feo'
    n.reload
  ...
  def test_multiple_users_can_manage_to_publish_a_content_if_reaches_1_0
  ...
  def test_multiple_users_can_manage_to_deny_a_content_if_reaches_minus_1_0
    test_users_exp_increases_when_a_content_he_accepted_is_published
    @mralariko = User.find_by_login('mralariko')
    assert_not_nil @mralariko
    maximize_exp(@mralariko)
  ...
  def test_users_faith_increases_when_making_a_publishing_decision
  ...
  def test_users_faith_points_doesnt_increase_with_incorrect_publishing_decisions
  ...
  def test_users_exp_behaves_as_expected_exp_function_while_hits_and_misses_are_modified
  end
  
  def test_last_editor_must_overturn_any_previous_publishing_decision
