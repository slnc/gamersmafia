...
class OrganizationsTest < Test::Unit::TestCase
  ...
  def test_find_organization_by_content_should_work_with_districts_content
    n65 = News.find(65)
  ...
  def test_find_organization_by_content_should_work_with_categorizable_content_in_no_organization
    n66 = News.find(66)
    assert Organizations.find_by_content(n66).nil?
  end
  
  def test_find_organization_by_content_should_work_with_noncategorizable
    assert Organizations.find_by_content(Funthing.find(1)).nil?
  end
  
  def test_change_organization_type_to_same
  ...
  def test_should_change_organization_type_from_faction_to_district
  ...
  def test_should_not_change_organization_type_if_not_faction
    # TEMP
    assert_raises(RuntimeError) do
