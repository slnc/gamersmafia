...
class HasBankAccountTest < Test::Unit::TestCase
  def setup
  ...
  def test_should_increment_money_if_added_money
  ...
  def test_should_raise_error_if_trying_to_move_negative_ammount
    d = HasBankAccountRecord.create({:name => 'foo'})
    assert_raises(Bank::NegativeAmmountError) { d.add_money(-1) }
    assert_raises(Bank::NegativeAmmountError) { d.remove_money(-1) }
  end

  def test_should_increment_money_if_remove_money
    d = HasBankAccountRecord.create({:name => 'foo'})
    p_cash = d.cash
    d.remove_money(1)
    assert d.cash == p_cash - 1
