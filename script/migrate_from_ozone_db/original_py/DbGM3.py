...
class DbGM3(object):
    ...
    def insert(self, table, dict_vals, returnid=False):
        ...
        for i in dict_vals:
            ...
            if dict_vals[i].__class__ == AttrFile:
                ...
                try:
                    list_vals.append(dict_vals[i].encode('utf8'))
                except UnicodeDecodeError:
                    list_vals.append(dict_vals[i].decode('utf8').encode('utf8'))
    ...
    def triggerConnection(self):
        if not self.db_conn:
            try:
                self.db_conn = psycopg2.connect('dbname=gamersmafia user=postgres password= host=localhost port=5432')
