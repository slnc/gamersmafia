...
for dbgame in DbGM3.query('select name, code from games'):
    newid = DbGM3.insert('interviews_categories', {'name': dbgame['name'], 'code': dbgame['code']}, True)
    DbGM3.query('update interviews_categories set toplevel_id = %s where name = \'%s\'' % (newid, dbgame['name'].replace('\'', '\\\'')))
...
gm3_ctype = DbGM3.query_single('SELECT id from content_types where name = \'interview\'')['id']
...
for dbsubcommunity in Db.query('select a.id, b.code from subcommunities a join games b on a.game_id = b.id where type_id = 1;'):
    for dbcel in Db.query('select * from celements_interviews where subcommunity_id = %i' % dbsubcommunity['id']):
        ...
        newcel = {'id': dbcel['id'], 
                  ...
                  'text': dbcel['content'],
                  'interviews_category_id': relbasecat_id[dbsubcommunity['code']],
                  'user_id': dbcel['author_user_id'],
                  'approved_by_user_id': get_approved_by_for('interviews', dbcel['id']),
        ...
        DbGM3.insert('interviews', newcel)
        ...
        # comments
        i = 0
        for dbcomment in Db.query('SELECT * FROM comments_celements_interviews where item_id = %i' % dbcel['id']):
...
restart_id = DbGM3.query_single('''select max(id) from interviews''')['max']
DbGM3.query('''alter sequence interviews_id_seq RESTART %i''' % (restart_id+1))
