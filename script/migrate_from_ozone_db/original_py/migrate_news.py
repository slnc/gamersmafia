...
for dbgame in DbGM3.query('select code from games'):
    newid = DbGM3.insert('news_categories', {'name': dbgame['code']}, True)
...
gm3_ctype = DbGM3.query_single('SELECT id from content_types where name = \'news\'')['id']
...
for dbsubcommunity in Db.query('select a.id, b.code from subcommunities a join games b on a.game_id = b.id where type_id = 1;'):
    ...
    for dbcat in Db.query('select * from celements_news_category where subcommunity_id = %i' % dbsubcommunity['id']):
        secondlvlcat_id = DbGM3.insert('news_categories', {'name': dbcat['name'], 'parent_id': relbasecat_id[dbsubcommunity['code']]}, True)
        secondlvlcats[dbcat['id']] = secondlvlcat_id

        for dbcel in Db.query('select * from celements_news where category_id = %i' % dbcat['id']):
            newcel = {'id': dbcel['id'], 
                      ...
                      'news_category_id': secondlvlcat_id,
                      'user_id': dbcel['author_user_id'],
                      'approved_by_user_id': get_approved_by_for('news', dbcel['id']),
            ...
            # comments
            i = 0
            for dbcomment in Db.query('SELECT * FROM comments_celements_news where item_id = %i' % dbcel['id']):
...
restart_id = DbGM3.query_single('''select max(id) from news''')['max']
DbGM3.query('''alter sequence news_id_seq RESTART %i''' % restart_id)
