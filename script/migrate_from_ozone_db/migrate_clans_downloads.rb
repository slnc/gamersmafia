...
ctype_id = ContentType.find_by_name('ClansDownload').id
Clan.db_query("DELETE FROM clans_downloads")
ClansDownload  # necesario para que cargue el modelo y luego la categoría

Oz.db_query("select a.* from celements_files a join subcommunities b on a.subcommunity_id = b.id where subcommunity_id IN (select id from subcommunities where type_id = 2)").each do |dbn|
  ...
  oldgallery = Oz.db_query("SELECT name FROM celements_files_category WHERE id = #{dbn['category_id']}")[0]['name']
  gal = ClansDownloadsCategory.find(:first, :conditions => "clan_id = #{clan_id} AND name = #{safe_str(oldgallery)}")
  gal = ClansDownloadsCategory.create({:name => oldgallery, :clan_id => clan_id}) if gal.nil?


  Clan.db_query("INSERT INTO clans_downloads(id, 
                                        created_on, 
                                        updated_on, 
                                        user_id, 
                                        hits_registered, 
  ...
  # comments
  Oz.db_query("SELECT * FROM comments_celements_files where item_id = #{dbn['id']}").each do |dbc|
...
max = Clan.db_query("select max(id) FROM clans_downloads")[0]['max'].to_i + 1
Clan.db_query("alter sequence clans_downloads_id_seq RESTART #{max}")
