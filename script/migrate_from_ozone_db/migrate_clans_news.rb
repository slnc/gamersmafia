...
ctype_id = ContentType.find_by_name('ClansNews').id
Clan.db_query("DELETE FROM clans_news")

Oz.db_query("select a.* from celements_news a join subcommunities b on a.subcommunity_id = b.id where subcommunity_id IN (select id from subcommunities where type_id = 2)").each do |dbn|
  ...
  Clan.db_query("INSERT INTO clans_news(id, 
                                        created_on, 
                                        updated_on, 
                                        user_id, 
                                        hits_registered, 
                                ...
                                VALUES (#{dbn['id']},
                                        ...
                                        #{Cms::PUBLISHED},
                                        #{clan_id},
                                        #{safe_str(dbn['headline'])},
                                        #{safe_str(dbn['text'].to_s + dbn['text_extra'].to_s)})")
  ...
  new_content_id = Content.db_query("INSERT INTO contents(content_type_id, external_id, updated_on, name, comments_count, is_public, state, clan_id)
                                                   VALUES(#{ctype_id}, #{dbn['id']}, '#{dbn['lastupdate_tstamp']}', #{safe_str(dbn['headline'])}, #{dbn['_cache_comments_count'] || 0}, 't', #{Cms::PUBLISHED}, #{clan_id}); SELECT id FROM contents order by id desc limit 1;")[0]['id']


  # comments
  Oz.db_query("SELECT * FROM comments_celements_news where item_id = #{dbn['id']}").each do |dbc|
...
max = Clan.db_query("select max(id) FROM clans_news")[0]['max'].to_i + 1
Clan.db_query("alter sequence clans_news_id_seq RESTART #{max}")
