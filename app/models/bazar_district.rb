class BazarDistrict < ActiveRecord::Base
  ...
  def rename_everything_if_name_or_code_changed
    if (slnc_changed?(:name) || slnc_changed?(:code)) && slnc_changed_old_values[:code].to_s != ''
      val = slnc_changed_old_values[:code] ? slnc_changed_old_values[:code] : slnc_changed_old_values[:name]
      field = slnc_changed_old_values[:code] ? :code : :name
      BazarDistrictPortal.send("find_by_#{field}",(val)).update_attributes(:name => self.name, :code => self.code)
