class Poll < ActiveRecord::Base
  ...
  def fix_solapping_polls
    ...
    solapping = self.main_category.poll.find(:first, :conditions => ["polls.id <> ?  AND contents.state = #{Cms::PUBLISHED} AND ((starts_on BETWEEN ? AND ?) OR (ends_on BETWEEN ? AND ?) OR (starts_on < ? AND ends_on > ?))", self.id, self.starts_on, self.ends_on, self.starts_on, self.ends_on, self.starts_on, self.ends_on])
  ...
  def check_start_ends
    ...
    if self.starts_on >= self.ends_on
      self.errors.add(
          "starts_on",
          "La fecha de comienzo debe ser anterior a la fecha de finalización" +
          " #{starts_on} #{ends_on}")
      false
    elsif starts_on <= Time.now and state <= Cms::PENDING
      self.errors.add(
          "starts_on",
          "La fecha de comienzo debe ser posterior a la fecha y hora actuales" +
          " #{starts_on}")
      false
    else
      true
