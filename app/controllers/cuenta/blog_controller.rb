...
class Cuenta::BlogController < ApplicationController
  ...
  def edit
    ...
    raise AccessDenied unless Authorization.can_edit_content?(@user, @blogentry)
    @title = "Editar entrada de blog \"#{@blogentry.title}\""
  end

  def update
    @blogentry = Blogentry.find(params[:id])
    raise ActiveRecord::RecordNotFound unless (user_is_authed && Authorization.can_edit_content?(@user, @blogentry))
  ...
  def destroy
    @blogentry = Blogentry.find(params[:id])
    raise ActiveRecord::RecordNotFound unless (user_is_authed and Authorization.can_edit_content?(@user, @blogentry))
