    ...
    if ban
      ...
      if User.count(:conditions => ['ipaddr = ?', request.remote_ip]) > 1
        ...
        SlogEntry.create({:type_id => SlogEntry::TYPES[:security], 
          :headline => "Registro desde IP existente <strong><a href=\"#{gmurl(@newuser)}\">#{@newuser.login}</a></strong> (#{request.remote_ip}): "<< (User.find(:all, :conditions => ['ipaddr = ? and id <> ?', request.remote_ip, @newuser.id]).collect {|u| "#{member_state(u.hstate)}<a href=\"#{gmurl(u)}\">#{u.login}</a>"}).join(', ')})
