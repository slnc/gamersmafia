class Admin::ContenidosController < ApplicationController
  ...
  def papelera
    ...
    for c in Cms::contents_classes_publishable + [Topic]
      @contents<< [Cms::translate_content_name(c.name).capitalize, b.send(Inflector::underscore(c.name)).find(:deleted, :conditions => 'updated_on > now() - \'1 month\'::interval'), Cms::translate_content_name(c.name)]
    end
  end
